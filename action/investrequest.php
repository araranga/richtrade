...
function trans()
{
	...
	if($_POST['submit']!='')
	{
		...
		$success = 1;
		$trans = trans();
		$convtotal_deduct = $_POST['withdraw']  + ($_POST['withdraw'] * (systemconfig("investpercent") / 100));
		
			$currentDate = new DateTime(); // current date and time
			$currentDate->modify('+90 days'); // add 90 days
			// Format the date and time for MySQL DATETIME
			$mysqlDateTime = $currentDate->format('Y-m-d H:i:s');		
		
		
		mysql_query_md("INSERT INTO tbl_invest_history SET payout_date='$mysqlDateTime',conv='$convtotal_deduct',transnum='$trans',claimtype='".$_POST['claimtype']."',address='".$_POST['address']."',accounts_id='$accounts_id',new_balance='".$sum."',amount='".$_POST['withdraw']."',current_balance='".$row['balance']."'");
		$q = mysql_query_md("SELECT * FROM tbl_accounts WHERE accounts_id='$accounts_id'");
...
//
?>
<h2>Investment Request - Balance(<?php echo number_format($row['balance_money'],2);?>)</h2>   
<?php
if($error!='')
{
?>
...
*Invest now and ensure (<?php echo systemconfig("investpercent"); ?>)% profit in 90 days!.
</div>
...
{
?>
<div class="noti"><ul class="fa-ul"><li><i class="fa fa-check fa-li"></i>Done requesting for investment please see status <a href='?pages=investhistory'>here</a> </li></ul></div>
<?php
}
?>
<?php
